...
from nets.resnet import ImageClinicalBasedSurv
...
parser.add_argument(
    "--root_path", type=str, default="../dataset/SMU/", help="Name of Experiment"
)
parser.add_argument(
    "--exp",
    type=str,
    default="ImageClinicalBasedSurv_DFS_Reproduce",
    help="experiment_name",
)
parser.add_argument("--model", type=str, default="resnet18", help="model_name")
parser.add_argument(
    "--num_classes", type=str, default="two", help="output channel of network"
)
parser.add_argument(
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # loss = L2_Regu_loss(weights=outputs[1], alpha=0.1) + \
            #     Loglike_loss(label_batch, outputs[2], n_interval)
            loss = NegativeLogLikelihood(
                -outputs[-1], label_batch[:, 0], label_batch[:, 1]
            ) + L2_Regu_loss(weights=outputs[1], alpha=0.1)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if iter_num > 0 and iter_num % 20 == 0:
                valid_cindex = validate_DFS_Reg(model, valloader, num_classes)
                ...
                if valid_cindex > best_cindex:
                    ...
                    save_mode_path = os.path.join(
                        snapshot_path,
                        "iter_{}_cindex_{}.pth".format(iter_num, round(best_cindex, 4)),
                    )
                ...
                logging.info("iteration %d : cindex : %f " % (iter_num, valid_cindex))
                model.train()
...
if __name__ == "__main__":
    ...
    snapshot_path = "../model/{}/{}_{}".format(args.exp, args.model, args.num_classes)
