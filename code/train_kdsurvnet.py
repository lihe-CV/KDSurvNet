...
from nets.resnet import resnet18, ImageClinicalIHCBasedSurv, ImageBasedSurv, ImageClinicalBasedSurv
...
parser.add_argument('--exp', type=str,
                    default='ImageClinicalIHCBasedSurv2ImageClinicalBased_KD_DFS', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    print(num_classes)
    teacher_model = ImageClinicalIHCBasedSurv(interval=n_interval, image_feature_length=1000, radiomics_feature_length=584,
                                              clinical_feature_length=9, ihc_feature_length=8, feature_planes=128).cuda()
    teacher_mode_path = "../model/ImageClinicalIHCBasedSurv_DFS/resnet18_two/resnet18_best_model.pth"
    teacher_model.load_state_dict(torch.load(teacher_mode_path))
    print("init weight from {}".format(teacher_mode_path))
    teacher_model.eval()

    model = ImageClinicalBasedSurv(interval=n_interval, image_feature_length=1000, radiomics_feature_length=584,
                                   clinical_feature_length=9, ihc_feature_length=8, feature_planes=128).cuda()
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                teacher_outputs = teacher_model(
                    volume_batch, radimocis, clinical, ihc)

            outputs = model(volume_batch, radimocis, clinical, ihc)

            alpha = 1
            T = 10
            KD_loss = nn.KLDivLoss()(F.log_softmax(outputs[2]/T, dim=1),
             F.softmax(teacher_outputs[2]/T, dim=1)) * (alpha * T * T) + torch.mean((teacher_outputs[0] - outputs[0])**2)

            # KD_loss = nn.SmoothL1Loss()(teacher_outputs[0], outputs[0])
            # Tar = torch.tensor([1, -1]*batch_size)
            # KD_loss = nn.SmoothL1Loss()(outputs[0], teacher_outputs[0])

            loss = NegativeLogLikelihood(-outputs[2], label_batch[:, 0],
                                         label_batch[:, 1]) + factor * KD_loss + L2_Regu_loss(weights=outputs[1], alpha=0.1)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
